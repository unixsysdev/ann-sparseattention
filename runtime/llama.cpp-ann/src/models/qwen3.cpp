...
void llama_model_qwen3::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (hparams.indexer_head_size > 0) {
            layer.indexer_proj   = create_tensor(tn(LLM_TENSOR_INDEXER_PROJ,   "weight", i), {n_embd, hparams.indexer_head_size}, TENSOR_NOT_REQUIRED);
            layer.indexer_attn_k = create_tensor(tn(LLM_TENSOR_INDEXER_ATTN_K, "weight", i), {n_embd, hparams.indexer_head_size}, TENSOR_NOT_REQUIRED);
        }
...
llama_model_qwen3::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            ggml_tensor * Qsearch = nullptr;
            ggml_tensor * Ksearch = nullptr;
            if (model.layers[il].indexer_proj && model.layers[il].indexer_attn_k) {
                Qsearch = ggml_mul_mat(ctx0, model.layers[il].indexer_proj, cur);
                Ksearch = ggml_mul_mat(ctx0, model.layers[il].indexer_attn_k, cur);
                cb(Qsearch, "Qsearch", il);
                cb(Ksearch, "Ksearch", il);
            }

            cur = build_attn_ann(inp_attn,
                    model.layers[il].wo, model.layers[il].wo_b, model.layers[il].wo_s,
                    Qcur, Kcur, Vcur, Qsearch, Ksearch,
