...
llama_kv_cache::llama_kv_cache(
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        const bool has_s = hparams.indexer_head_size > 0;

        ggml_tensor * k = has_k ? ggml_new_tensor_3d(ctx, type_k, n_embd_k_gqa, kv_size, n_stream) : nullptr;
        ggml_tensor * v = has_v ? ggml_new_tensor_3d(ctx, type_v, n_embd_v_gqa, kv_size, n_stream) : nullptr;
        ggml_tensor * s_cache = has_s ? ggml_new_tensor_3d(ctx, type_k, hparams.indexer_head_size, kv_size, n_stream) : nullptr;

        has_k && ggml_format_name(k, "cache_k_l%d", il);
        has_v && ggml_format_name(v, "cache_v_l%d", il);
        has_s && ggml_format_name(s_cache, "cache_s_l%d", il);

        std::vector<ggml_tensor *> k_stream;
        std::vector<ggml_tensor *> v_stream;
        std::vector<ggml_tensor *> s_stream;

        for (uint32_t s = 0; s < n_stream; ++s) {
            k_stream.push_back(has_k ? ggml_view_2d(ctx, k, n_embd_k_gqa, kv_size, k->nb[1], s*k->nb[2]) : nullptr);
            v_stream.push_back(has_v ? ggml_view_2d(ctx, v, n_embd_v_gqa, kv_size, v->nb[1], s*v->nb[2]) : nullptr);
            s_stream.push_back(has_s ? ggml_view_2d(ctx, s_cache, hparams.indexer_head_size, kv_size, s_cache->nb[1], s*s_cache->nb[2]) : nullptr);
        }

        map_layer_ids[il] = layers.size();

        layers.push_back({ il, k, v, s_cache, k_stream, v_stream, s_stream, });
    }
...

    {
        ...
        LLAMA_LOG_INFO("%s: size = %7.2f MiB (%6u cells, %3d layers, %2u/%u seqs), K (%s): %7.2f MiB, V (%s): %7.2f MiB, S (%s): %7.2f MiB\n", __func__,
                (float)(memory_size_k + memory_size_v + memory_size_s) / (1024.0f * 1024.0f), kv_size, (int) layers.size(), n_seq_max, n_stream,
                ggml_type_name(type_k), (float)memory_size_k / (1024.0f * 1024.0f),
                ggml_type_name(type_v), (float)memory_size_v / (1024.0f * 1024.0f),
                ggml_type_name(type_k), (float)memory_size_s / (1024.0f * 1024.0f));
    }
...
ggml_type llama_kv_cache::type_s() const {
    return layers[0].s ? layers[0].s->type : GGML_TYPE_COUNT;
}
...
ggml_tensor * llama_kv_cache::get_s(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
    ...
    const uint64_t kv_size = get_size();
    const uint64_t n_embd_s = s->ne[0];
    const uint32_t ns = sinfo.s1 - sinfo.s0 + 1;

    return ggml_view_4d(ctx, s,
            n_embd_s, n_kv, ns, 1,
            ggml_row_size(s->type, n_embd_s),
            ggml_row_size(s->type, n_embd_s*kv_size),
            ggml_row_size(s->type, n_embd_s*kv_size)*ns,
            ggml_row_size(s->type, n_embd_s*kv_size)*sinfo.s0);
}
...
ggml_tensor * llama_kv_cache::cpy_s(ggml_context * ctx, ggml_tensor * s_cur, ggml_tensor * s_idxs, int32_t il, const slot_info & sinfo) const {
    GGML_UNUSED(sinfo);

    const int32_t ikv = map_layer_ids.at(il);

    ggml_tensor * s = layers[ikv].s;
    GGML_ASSERT(s != nullptr);

    const int64_t n_embd_s = s_cur->ne[0];
    const int64_t n_tokens = s_cur->ne[1];

    GGML_ASSERT(n_embd_s == s->ne[0]);

    const int64_t n_stream = s->ne[2];

    if (n_stream > 1) {
        const int64_t kv_size = get_size();
        s = ggml_reshape_2d(ctx, s, n_embd_s, kv_size*n_stream);
    }

    return ggml_set_rows(ctx, s, s_cur, s_idxs);
}
...
size_t llama_kv_cache::size_s_bytes() const {
    size_t size_s_bytes = 0;

    for (const auto & layer : layers) {
        size_s_bytes += layer.s ? ggml_nbytes(layer.s) : 0;
    }
...
ggml_type llama_kv_cache_context::type_s() const {
    return kv->type_s();
}
...
ggml_tensor * llama_kv_cache_context::get_s(ggml_context * ctx, int32_t il) const {
    return kv->get_s(ctx, il, n_kv, sinfos[i_cur]);
}
...
ggml_tensor * llama_kv_cache_context::cpy_s(ggml_context * ctx, ggml_tensor * s_cur, ggml_tensor * s_idxs, int32_t il) const {
    return kv->cpy_s(ctx, s_cur, s_idxs, il, sinfos[i_cur]);
}
