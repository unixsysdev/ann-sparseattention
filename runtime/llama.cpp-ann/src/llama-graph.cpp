...
static float llama_ann_tensor_get_f32(const ggml_tensor * t, int64_t i0, int64_t i1 = 0, int64_t i2 = 0, int64_t i3 = 0) {
    ...
    switch (t->type) {
        ...
        default:
            GGML_ABORT("llama_ann_hnsw_topk: unsupported tensor type");
    }
}

static void llama_ann_hnsw_topk_op(ggml_tensor * dst, int ith, int nth, void * userdata) {
    GGML_UNUSED(nth);
    if (ith != 0) {
        return;
    }

    const auto * params = (const llama_ann_hnsw_params *) userdata;
    ...
    GGML_ASSERT(s->ne[2] == 1 && s->ne[3] == 1);
    GGML_ASSERT(mask->type == GGML_TYPE_F32);

    const int d    = (int) q->ne[0];
    const int n_kv = (int) s->ne[1];
    const int k    = std::min(params->k, n_kv);
    ...
    for (int j = 0; j < n_kv; ++j) {
        const float m = llama_ann_tensor_get_f32(mask, j, 0, 0, 0);
        if (std::isfinite(m)) {
            labels.push_back(j);
        }
    }
    ...
    const int valid = (int) labels.size();
    const int k_eff = std::min(k, valid);

    std::vector<float> qv(d);
    for (int i = 0; i < d; ++i) {
        qv[i] = llama_ann_tensor_get_f32(q, i, 0, 0, 0);
    }

    std::vector<float> data((size_t) valid * d);
    for (int r = 0; r < valid; ++r) {
        const int j = labels[r];
        float * row = data.data() + (size_t) r * d;
        for (int i = 0; i < d; ++i) {
            row[i] = llama_ann_tensor_get_f32(s, i, j, 0, 0);
        }
    }

    hnswlib::InnerProductSpace space(d);
    hnswlib::HierarchicalNSW<float> index(&space, valid, params->m, params->ef_construction, 100);
    index.setEf(params->ef_search);
    for (int r = 0; r < valid; ++r) {
        index.addPoint(data.data() + (size_t) r * d, r);
    }

    auto result = index.searchKnn(qv.data(), k_eff);
    int pos = k_eff - 1;
    while (!result.empty() && pos >= 0) {
        out[pos--] = labels[(int) result.top().second];
        result.pop();
    }
    for (; pos >= 0; --pos) {
        out[pos] = out[pos + 1];
...
ggml_tensor * llm_graph_context::build_attn_ann(
        llm_graph_input_attn_kv * inp,
        ggml_tensor * wo,
        ggml_tensor * wo_b,
        ggml_tensor * wo_s,
        ...
        ggml_tensor * k_cur,
        ggml_tensor * v_cur,
        ggml_tensor * q_search_cur,
        ggml_tensor * k_search_cur,
        ggml_tensor * kq_b,
        ggml_tensor * sinks,
        ggml_tensor * v_mla,
            float     kq_scale,
...

    {
        ...
        if (k_search_cur && mctx_cur->get_s(ctx0, il)) {
            ggml_build_forward_expand(gf, mctx_cur->cpy_s(ctx0, k_search_cur, k_idxs, il));
        }
    }
    ...
    const bool have_search = q_search_cur && k_search_cur && mctx_cur->get_s(ctx0, il);
    const bool decode_one  = n_tokens == 1 && q_search_cur && q_search_cur->ne[1] == 1;
    const bool v_trans     = v->nb[1] > v->nb[2];
    const int64_t ann_k    = hparams.indexer_top_k > 0 ? hparams.indexer_top_k : 128;

    if (have_search && decode_one && !v_trans && kq_b == nullptr && ann_k > 0 && mctx_cur->get_n_kv() >= (uint32_t) ann_k) {
        ggml_tensor * s = mctx_cur->get_s(ctx0, il); // [d_search, n_kv, 1, 1] for a single stream
        ggml_tensor * scores = ggml_mul_mat(ctx0, s, q_search_cur); // [n_kv, 1, 1, 1]
        ggml_mul_mat_set_prec(scores, GGML_PREC_F32);
        scores = ggml_add(ctx0, scores, inp->self_kq_mask);
        cb(scores, "ann_scores", il);

        ggml_tensor * top = nullptr;
        const char * ann_search = std::getenv("LLAMA_ANN_SEARCH");
        if (ann_search && std::strcmp(ann_search, "hnsw") == 0) {
            auto * hnsw_params = new llama_ann_hnsw_params {
                /*.k               =*/ (int) ann_k,
                /*.m               =*/ 16,
                /*.ef_construction =*/ 64,
                /*.ef_search       =*/ 64,
            };
            ggml_tensor * args[] = { q_search_cur, s, inp->self_kq_mask };
            top = ggml_custom_4d(ctx0, GGML_TYPE_I32, ann_k, 1, 1, 1, args, 3, llama_ann_hnsw_topk_op, 1, hnsw_params);
            ggml_backend_sched_set_tensor_backend(sched, top, backend_cpu);
        } else {
            top = ggml_top_k(ctx0, scores, ann_k); // [K, 1, 1, 1]
        }
        cb(top, "ann_topk", il);

        ggml_tensor * k_rows = ggml_reshape_4d(ctx0, k, k->ne[0]*k->ne[1], k->ne[2], 1, 1);
        ggml_tensor * v_rows = ggml_reshape_4d(ctx0, v, v->ne[0]*v->ne[1], v->ne[2], 1, 1);

        k = ggml_get_rows(ctx0, k_rows, top);
        v = ggml_get_rows(ctx0, v_rows, top);

        k = ggml_reshape_4d(ctx0, k, hparams.n_embd_head_k(il), hparams.n_head_kv(il), ann_k, 1);
        v = ggml_reshape_4d(ctx0, v, hparams.n_embd_head_v(il), hparams.n_head_kv(il), ann_k, 1);

        ggml_tensor * cur = build_attn_mha(q, k, v, nullptr, nullptr, sinks, v_mla, kq_scale, il);
        cb(cur, "ann_kqv_out", il);
